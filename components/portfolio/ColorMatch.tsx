...
interface ColorOption { h: number; s: number; l: number }
...
export function ColorMatch({ onClose }: { onClose: () => void }) {
  ...
  const endGame = useCallback((finalScore: number) => {
    ...
    setBest((prev) => {
      if (finalScore > prev) {
        setNewRecord(true);
        try { localStorage.setItem(LS_KEY, String(finalScore)); } catch { /**/ }
        return finalScore;
      }
      return prev;
    });
  ...
  const handlePick = useCallback((opt: ColorOption) => {
                ...
                <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div className="flex gap-2">
                    {["#f59e0b","#8b5cf6","#10b981","#ef4444"].map((c,i) => (
                      <motion.div key={c} animate={{ y: [0,-6,0] }} transition={{ duration: 1, delay: i*0.15, repeat: Infinity }}
                        className="h-10 w-10 rounded-xl shadow-lg" style={{ background: c }} />
                    ))}
                  </div>
                  <p className="text-lg font-black text-foreground">Match the color</p>
                  <p className="text-sm text-muted-foreground max-w-[240px]">
                    A target swatch appears — tap the matching one from 4 options before time runs out.
                  </p>
              ...
              {(phase === "playing" || phase === "wrong") && current && (
                <motion.div key="game" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted/50">
                    <motion.div
                      className={cn("h-full rounded-full transition-colors",
                        timeLeft > 0.5 ? "bg-emerald-500" : timeLeft > 0.25 ? "bg-amber-500" : "bg-red-500")}
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Match this color</p>
                    <motion.div
                      key={round}
                      initial={{ scale: 0.8, opacity: 0 }}
                      animate={flash === "correct" ? { scale: [1,1.12,1], borderColor: "#10b981" } :
                               flash === "wrong"   ? { scale: [1,0.9,1],  borderColor: "#ef4444" } :
                               { scale: 1 }}
                      className="h-24 w-24 rounded-2xl border-4 border-transparent shadow-xl"
                      style={{ background: hsl(current.target.h, current.target.s, current.target.l) }}
                    />
                  </div>
