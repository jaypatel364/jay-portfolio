...
const QUESTIONS: Question[] = [
  {
    ...
    a: {
      ...
      emoji: "🌑",
      scores: { midnight_architect: 2, rubber_duck_whisperer: 1 },
    },
    b: {
      label: "Light mode (I like pain)",
      emoji: "☀️",
      scores: { pixel_perfectionist: 2, ship_it_king: 1 },
    },
  },
  {
    q: "A bug has been haunting you for 3 hours. You:",
    a: {
      label: "Rubber duck it until it confesses",
      emoji: "🦆",
      scores: { rubber_duck_whisperer: 3 },
    },
  ...
  },
  {
    q: "Tabs or spaces?",
    a: {
      label: "Tabs. Obviously.",
      emoji: "⇥",
      scores: { midnight_architect: 1, caffeine_compiler: 2 },
    },
  ...
  },
  {
    q: "It's 2am. You're:",
    a: {
      label: "Rewriting the entire codebase from scratch",
      emoji: "🌙",
      scores: { midnight_architect: 3 },
    },
    b: {
      label: "Finally understanding that one regex",
      emoji: "🧠",
      scores: { open_source_oracle: 3 },
    },
  },
  {
    q: "Your relationship with coffee:",
    a: {
      label: "It's basically blood at this point",
      emoji: "☕",
      scores: { caffeine_compiler: 3 },
    },
    b: {
      label: "I run on pure stubbornness",
      emoji: "💪",
      scores: { ship_it_king: 2, midnight_architect: 1 },
    },
  },
  {
    q: "You open a new codebase. First thing:",
    a: {
      label: "Read every file before touching anything",
      emoji: "📚",
      scores: { open_source_oracle: 2, pixel_perfectionist: 1 },
    },
    b: {
      label: "Find main() and start hacking",
      emoji: "⚡",
      scores: { ship_it_king: 2, caffeine_compiler: 1 },
    },
  },
];
...
const ARCHETYPES: Record<Archetype, ArchetypeInfo> = {
  ...
  ship_it_king: {
    ...
    traits: [
      ...
      "Todo comments since 2019",
      "Closes 10 issues before lunch",
      "Types fast, thinks faster",
    ],
  ...
  open_source_oracle: {
    ...
    traits: [
      ...
      "Has opinions about semver",
      "The team's Stack Overflow",
    ],
...
function tally(answers: Choice[]): Archetype {
  const scores: Record<Archetype, number> = {
    ...
    rubber_duck_whisperer: 0,
    caffeine_compiler: 0,
    open_source_oracle: 0,
  };
  ...
  const share = () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      ...
      toast("📋 Copied to clipboard!", {
        description: "Share your archetype with the world.",
        duration: 3000,
      });
                ...
                >
                  ...
                  <div>
                    <p className="font-heading text-xl font-black text-foreground">
                      What's your dev vibe?
                    </p>
                    <p className="mt-2 text-sm text-muted-foreground leading-relaxed max-w-[280px] mx-auto">
                      6 quick "this or that" choices. We'll reveal your developer archetype with a
                      shareable badge.
                    </p>
                  </div>
              ...
              {phase === "result" && info && (
                ...
                >
                  ...
                  <div className="relative w-full overflow-hidden rounded-2xl border border-border/50 bg-card/80 p-5">
                    <div
                      className="pointer-events-none absolute -right-6 -top-6 h-32 w-32 rounded-full blur-3xl opacity-25"
                  ...
                  <div className="flex w-full flex-col gap-2 mt-1">
                    ...
                    >
                      <Mail className="h-3.5 w-3.5" /> Hire the{" "}
                      {info.name.split(" ").slice(1).join(" ")} <ArrowRight className="h-3 w-3" />
                    </motion.button>
                  </div>
                </motion.div>
              )}
